...
# 方法1：from_template（最简单，推荐）
template = PromptTemplate.from_template("你的模块文本 {变量名}")
...
# 返回格式化后的字符串
prompt_str = template.format(name="Nike")
print(prompt_str) # Hello Nike.
...
### 方式2：invoke()-返回PromptValue

```python
template = PromptTemplate.from_template("你好 {name}")

# 返回 PromptValue 对象
prompt_value = template.invoke({'name':'Nike'})

# 获取文本
print(prompt_value.text) # 你好 Nike
```
...
### 方法3：使用MessagePromptTemplate(高级)

```python
from langchain_core.prompts import (SystemMessagePrompt, HumanMessagePrompt)
